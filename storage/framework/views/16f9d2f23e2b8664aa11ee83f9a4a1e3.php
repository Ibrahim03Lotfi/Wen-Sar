...
<div class="bg-gray-50 py-20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            <a href="<?php echo e(route('business.category', $cat->id)); ?>" class="group bg-white p-8 rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl hover:border-brand-green/20 transition-all text-center relative overflow-hidden">
                ...
                <div class="relative">
                    ...
                    <div class="w-20 h-20 bg-brand-green/10 rounded-2xl flex items-center justify-center mx-auto mb-5 group-hover:bg-brand-green transition-colors shadow-lg shadow-brand-green/5 group-hover:shadow-brand-green/20">
                        ...
                        <?php if(str_contains($cat->name, 'مطاعم') || str_contains($cat->name, 'مأكولات')): ?>
                        ...
                        <?php elseif(str_contains($cat->name, 'مقاهي') || str_contains($cat->name, 'كافيه')): ?>
                            <svg class="w-10 h-10 text-brand-green group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <?php elseif(str_contains($cat->name, 'محلات') || str_contains($cat->name, 'تجاري')): ?>
                            <svg class="w-10 h-10 text-brand-green group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/>
                            </svg>
                        <?php elseif(str_contains($cat->name, 'صحية') || str_contains($cat->name, 'طب') || str_contains($cat->name, 'صيدلية')): ?>
                            <svg class="w-10 h-10 text-brand-green group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/>
                            </svg>
                        <?php elseif(str_contains($cat->name, 'تعليم') || str_contains($cat->name, 'مدارس') || str_contains($cat->name, 'جامعة')): ?>
                            <svg class="w-10 h-10 text-brand-green group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l9-5-9-5-9 5 9 5z"/>
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z"/>
                            </svg>
                        <?php elseif(str_contains($cat->name, 'سياحة') || str_contains($cat->name, 'فنادق') || str_contains($cat->name, 'سفر')): ?>
                            <svg class="w-10 h-10 text-brand-green group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064"/>
                            </svg>
                        <?php elseif(str_contains($cat->name, 'خدمات') || str_contains($cat->name, 'صيانة')): ?>
                            <svg class="w-10 h-10 text-brand-green group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <?php elseif(str_contains($cat->name, 'رياضة') || str_contains($cat->name, 'صالات')): ?>
                            <svg class="w-10 h-10 text-brand-green group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064"/>
                            </svg>
                        <?php elseif(str_contains($cat->name, 'تقنية') || str_contains($cat->name, 'إلكترونيات') || str_contains($cat->name, 'موبايل')): ?>
                            <svg class="w-10 h-10 text-brand-green group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z"/>
                            </svg>
                        <?php elseif(str_contains($cat->name, 'سيارات') || str_contains($cat->name, 'معارض')): ?>
                            <svg class="w-10 h-10 text-brand-green group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <h3 class="font-bold text-gray-800 text-lg group-hover:text-brand-green transition-colors mb-2"><?php echo e($cat->name); ?></h3>
                    <div class="flex items-center justify-center gap-2">
                        <span class="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full"><?php echo e($cat->total_businesses_count); ?> مكان</span>
                    </div>
                </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\LENOVO\Desktop\Wen-Sar\resources\views\categories.blade.php ENDPATH**/ ?>