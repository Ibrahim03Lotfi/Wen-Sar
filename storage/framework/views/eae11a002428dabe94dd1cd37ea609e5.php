    ...
    <div class="relative max-w-7xl mx-auto px-4 py-16 md:py-24 lg:py-32 sm:px-6 lg:px-8 flex flex-col items-center text-center">
        ...
        <div x-data="searchForm()"
            ...
            <form action="<?php echo e(route('business.search')); ?>" method="GET" class="contents">
                ...
                <button type="submit"
                        :disabled="!canSearch"
                        :class="canSearch ? 'bg-brand-green hover:opacity-90 cursor-pointer' : 'bg-gray-400 cursor-not-allowed'"
...
<script>
    (function() {
        ...
        function updateSlideWidths() {
            ...
            slides.forEach(function(slide) { slide.style.width = width + '%'; });
        }
        ...
        function render() {
            ...
            dots.forEach(function(dot, index) {
                ...
                dot.setAttribute('data-index', Math.min(index, maxIndex()));
            });
        }

        function next() { current = current >= maxIndex() ? 0 : current + 1; render(); }
        function prev() { current = current <= 0 ? maxIndex() : current - 1; render(); }
        function startAuto() { stopAuto(); intervalId = setInterval(next, 5000); }
        ...
        dots.forEach(function(dot) {
            dot.addEventListener('click', function() {
                ...
                if (!Number.isNaN(index)) { current = index; render(); }
            });
        });
        ...
        window.addEventListener('resize', function() { updateSlideWidths(); render(); });
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
        ...
        <a href="<?php echo e(route('business.category', $cat->id)); ?>" class="group bg-white p-3 md:p-6 rounded-xl md:rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:border-brand-green/20 transition-all text-center active:scale-95">
            <div class="w-12 h-12 md:w-16 md:h-16 bg-brand-green/5 rounded-full flex items-center justify-center mx-auto mb-2 md:mb-4 group-hover:bg-brand-green transition-colors">
                <?php if(str_contains($cat->name, '????') || str_contains($cat->name, '????') || str_contains($cat->name, '??????')): ?>
                ...
                <?php elseif(str_contains($cat->name, '?????') || str_contains($cat->name, '?????')): ?>
                    <svg class="w-7 h-7 md:w-8 md:h-8 text-brand-green group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<script>
    function searchForm() {
        return {
            governorateId: <?php echo json_encode($governorates->first()?->id ?? '', 15, 512) ?>,
            districtId: '',
            categoryId: '',
            districts: <?php echo json_encode($governorates->first()?->districts->map(fn($d) => ['id' => $d->id, 'name' => $d->name])->values() ?? [], 512) ?>,
            governorates: <?php echo json_encode($governorates->map(fn($g) => ['id' => $g->id, 'name' => $g->name])->values(), 512) ?>,
            get canSearch() {
            ...
            async updateDistricts() {
                ...
                try {
                    const url = '/api/governorates/' + encodeURIComponent(this.governorateId) + '/districts';
                    ...
                    console.error('Error updating districts', err);
                    this.districts = [];
                    this.districtId = '';
                }
            }
