...
<?php endif; ?>
    ...
    <header class="bg-brand-white shadow-sm sticky top-0 z-50 border-b border-gray-100 will-change-transform" style="transform: translateZ(0);">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex h-16 md:h-20 items-center justify-between">
                ...
                <button onclick="var m=document.getElementById('mobile-menu'); m.style.display=m.style.display==='none'?'block':'none';" class="md:hidden p-2 rounded-lg text-gray-600 hover:bg-gray-100">
    ...
    <!-- Facebook Toast -->
    <div x-show="fbToast === true" x-cloak style="display:none !important" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 translate-y-2" class="fixed bottom-20 left-1/2 transform -translate-x-1/2 z-50">
    ...
    <!-- Terms Modal -->
    <div x-show="termsOpen === true" x-cloak style="display:none !important" class="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div x-show="termsOpen" class="fixed inset-0 bg-gray-900 bg-opacity-75" @click="termsOpen = false"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>
            <div x-show="termsOpen" class="inline-block align-bottom bg-white rounded-2xl text-right overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="mt-3 text-center sm:mt-0 sm:text-right w-full">
                        <h3 class="text-2xl font-bold text-gray-900 mb-6">شروط الاستخدام</h3>
                        <div class="mt-2 space-y-4 text-gray-600 max-h-96 overflow-y-auto">
                            ...
                            <div class="space-y-3">
                                <div><h4 class="font-bold text-gray-800 mb-2">1. قبول الشروط</h4><p class="text-sm">باستخدامك لمنصة "وين صار"، فإنك توافق على الالتزام بهذه الشروط والأحكام.</p></div>
                                <div><h4 class="font-bold text-gray-800 mb-2">2. استخدام المنصة</h4><p class="text-sm">يجب استخدام المنصة لأغراض مشروعة فقط.</p></div>
                                <div><h4 class="font-bold text-gray-800 mb-2">3. حسابات المستخدمين</h4><p class="text-sm">أنت مسؤول عن الحفاظ على سرية معلومات حسابك وكلمة المرور.</p></div>
                                <div><h4 class="font-bold text-gray-800 mb-2">4. المحتوى</h4><p class="text-sm">المحتوى المنشور على المنصة هو مسؤولية أصحابه.</p></div>
                                <div><h4 class="font-bold text-gray-800 mb-2">5. التعديلات</h4><p class="text-sm">نحتفظ بالحق في تعديل هذه الشروط في أي وقت.</p></div>
                            </div>
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <button type="button" @click="termsOpen = false" class="w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-4 py-2 bg-brand-green text-base font-medium text-white hover:opacity-90 focus:outline-none sm:ml-3 sm:w-auto sm:text-sm">فهمت</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- Privacy Modal -->
    <div x-show="privacyOpen === true" x-cloak style="display:none !important" class="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div x-show="privacyOpen" class="fixed inset-0 bg-gray-900 bg-opacity-75" @click="privacyOpen = false"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>
            <div x-show="privacyOpen" class="inline-block align-bottom bg-white rounded-2xl text-right overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="mt-3 text-center sm:mt-0 sm:text-right w-full">
                        <h3 class="text-2xl font-bold text-gray-900 mb-6">سياسة الخصوصية</h3>
                        <div class="mt-2 space-y-4 text-gray-600 max-h-96 overflow-y-auto">
                            <p>نحن في "وين صار" نولي أهمية كبيرة لخصوصيتك.</p>
                            <div class="space-y-3">
                                <div><h4 class="font-bold text-gray-800 mb-2">1. المعلومات التي نجمعها</h4><p class="text-sm">نجمع المعلومات التي تقدمها لنا مباشرة عند التسجيل أو التواصل معنا.</p></div>
                                <div><h4 class="font-bold text-gray-800 mb-2">2. كيفية استخدام المعلومات</h4><p class="text-sm">نستخدم معلوماتك لتقديم خدماتنا وتحسين تجربة المستخدم.</p></div>
                                <div><h4 class="font-bold text-gray-800 mb-2">3. حماية المعلومات</h4><p class="text-sm">نستخدم إجراءات أمنية مناسبة لحماية معلوماتك.</p></div>
                                <div><h4 class="font-bold text-gray-800 mb-2">4. مشاركة المعلومات</h4><p class="text-sm">لا نبيع أو نؤجر معلوماتك الشخصية لطرف ثالث.</p></div>
                                <div><h4 class="font-bold text-gray-800 mb-2">5. حقوقك</h4><p class="text-sm">لديك الحق في الوصول إلى معلوماتك الشخصية وتصحيحها أو حذفها.</p></div>
                            </div>
                            <p class="text-sm text-gray-500">آخر تحديث: 14 أبريل 2026</p>
                        </div>
                    </div>
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <button type="button" @click="privacyOpen = false" class="w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-4 py-2 bg-brand-green text-base font-medium text-white hover:opacity-90 focus:outline-none sm:ml-3 sm:w-auto sm:text-sm">فهمت</button>
                </div>
            </div>
        </div>
    </div>
...
</script><?php /**PATH C:\Users\LENOVO\Desktop\Wen-Sar\resources\views/layouts/app.blade.php ENDPATH**/ ?>