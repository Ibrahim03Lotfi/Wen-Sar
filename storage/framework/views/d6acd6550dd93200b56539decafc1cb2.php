...
<div class="max-w-7xl mx-auto px-4 pt-8 md:pt-12 sm:px-6 lg:px-8">
    <div
        x-data="{
            current: 0,
            total: <?php echo e(max($ads->count(), 1)); ?>,
            intervalId: null,
            init() {
    ...
    >
        <div class="overflow-hidden rounded-2xl md:rounded-3xl shadow-xl border border-gray-100 bg-white">
            <?php if($ads->count() > 0): ?>
                <div class="relative h-44 sm:h-56 md:h-72 lg:h-80">
            ...
            <?php else: ?>
                <div class="h-44 sm:h-56 md:h-72 lg:h-80 bg-gradient-to-r from-brand-green via-emerald-700 to-brand-green flex items-center justify-center px-6 text-center">
        ...
        <?php if($ads->count() > 1): ?>
        <button @click="prev()" type="button" class="absolute top-1/2 -translate-y-1/2 right-3 md:right-5 bg-white/90 hover:bg-white text-gray-700 w-9 h-9 md:w-11 md:h-11 rounded-full shadow-lg flex items-center justify-center transition-all">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
            </svg>
        </button>
        <button @click="next()" type="button" class="absolute top-1/2 -translate-y-1/2 left-3 md:left-5 bg-white/90 hover:bg-white text-gray-700 w-9 h-9 md:w-11 md:h-11 rounded-full shadow-lg flex items-center justify-center transition-all">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex justify-center mt-4 gap-2">
            ...
            <button
                ...
                :class="current === <?php echo e($index); ?> ? 'bg-brand-green scale-110' : 'bg-gray-300 hover:bg-gray-400'"
