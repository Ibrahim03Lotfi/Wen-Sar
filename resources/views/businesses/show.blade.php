                ...
                <div x-show="showLoginAlert" x-cloak
                    ...
                    <div x-show="showLoginAlert"
                        ...
                        <div class="flex flex-col sm:flex-row gap-3">
                            <a href="{{ route('select-role') }}" class="flex-1 bg-brand-green text-white font-bold py-3 px-6 rounded-xl hover:opacity-90 transition-all">تسجيل الدخول</a>
                            <a href="{{ route('select-role') }}?action=register" class="flex-1 bg-gray-100 text-gray-700 font-bold py-3 px-6 rounded-xl hover:bg-gray-200 transition-all">إنشاء حساب</a>
                        </div>
        ...
        <div class="lg:col-span-2 space-y-6 md:space-y-12 order-2 lg:order-1">
            ...
            <section class="bg-white p-5 md:p-10 rounded-2xl md:rounded-[2.5rem] shadow-sm border border-gray-50">
                ...
                <div class="text-center py-5 md:py-6 border rounded-2xl md:rounded-3xl mb-6 md:mb-10 bg-gray-50">
                    ...
                    <a href="{{ route('select-role') }}" class="text-brand-green font-bold underline text-sm md:text-base">تسجيل الدخول</a>
                </div>
                @endauth
                    ...
                    <div class="flex items-start gap-3 md:gap-5 p-3 md:p-4 rounded-xl md:rounded-2xl hover:bg-brand-white transition-colors">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="font-bold text-gray-800 text-base md:text-lg truncate" dir="ltr">
                                {{ $regularHours['open'] ?? ($business->opening_time ? substr($business->opening_time, 0, 5) : '--:--') }} - {{ $regularHours['close'] ?? ($business->closing_time ? substr($business->closing_time, 0, 5) : '--:--') }}
                            ...
                            <div class="mt-2">
                                ...
                                <div class="flex flex-wrap gap-1 mt-1">
                                    @foreach($closedDays as $day)
                                        <span class="text-xs bg-red-50 text-red-600 px-2 py-1 rounded">{{ $dayNames[$day] ?? $day }}</span>
                                    @endforeach
                                </div>
                            </div>
                            @endif
                            ...
                            <div class="mt-2 space-y-1">
                                ...
                                @foreach($overrides as $day => $times)
                                    <div class="flex items-center gap-2 text-sm">
                                        <span class="font-bold text-gray-700">{{ $dayNames[$day] ?? $day }}:</span>
                                        <span class="text-gray-600" dir="ltr">{{ $times['open'] ?? '--:--' }} - {{ $times['close'] ?? '--:--' }}</span>
                                    </div>
                                @endforeach
                            </div>
                            @endif
